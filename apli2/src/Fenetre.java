...
class BoutonSetAddCat implements ActionListener{
	public void actionPerformed(ActionEvent arg0){
		...
		else
		{
			...
			System.out.println("On va creer une nouvelle categorie qui aura pour nom "+ addCatNom.getText()+ " qui aura pour activites:");
			if(addActiviteNom1.getText() != null && (addActiviteNom1.getText()).length() == 0)
			{
			}
			else
			{
				listAdd.add(addActiviteNom1.getText());
			...
			{
				listAdd.add(addActiviteNom2.getText());
			...
			{
				listAdd.add(addActiviteNom3.getText());
			...
			{
				listAdd.add(addActiviteNom4.getText());
	...
	public void actionPerformed(ActionEvent arg0){
			...
			for(int i = 0 ;i<activite.size(); i++)
			{
				...
				if(m==n)
				{
					...
					{
						String S=(String) ((((Categorie) activite.get(i)).getArray()).get(j));
						JButton b = new JButton();
						b.setText(S);
						suboptions.add(b);
